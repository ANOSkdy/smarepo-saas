...
export default function StampCard({
  ...
  const handleCheckIn = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (workDescription) handleStamp('IN', workDescription);
  };
    ...
    return (
      <div className="w-full max-w-md rounded-lg bg-white p-8 text-center shadow-md">
        ...
        <form onSubmit={handleCheckIn} className="mt-6 space-y-4">
          <div>
            <label htmlFor="workDescription" className="block text-left text-sm font-medium text-gray-700">作業内容</label>
            <select id="workDescription" name="workDescription" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm" defaultValue="">
          ...
          <button type="submit" disabled={workTypes.length === 0} className="w-full rounded-md bg-blue-600 px-4 py-3 text-xl font-bold text-white hover:bg-blue-700 disabled:bg-gray-400">
            出 勤
          </button>
        </form>
        <LogoutButton />
  ...
  if (stampType === 'OUT') {
    return (
      <div className="w-full max-w-md rounded-lg bg-white p-8 text-center shadow-md">
        ...
        <p className="mt-2 text-lg text-gray-600">{userName} さん</p>
        <p className="mt-4 text-gray-800"><span className="font-bold">機械:</span> {machineName}</p>
        <p className="text-gray-800"><span className="font-bold">作業内容:</span> {lastWorkDescription || '（記録なし）'}</p>
        <button onClick={handleCheckOut} type="button" className="mt-8 w-full rounded-md bg-red-600 px-4 py-3 text-xl font-bold text-white hover:bg-red-700">
          退 勤
        </button>
        {/* 「出勤画面に戻る」リンクは削除しました */}
        <LogoutButton />
