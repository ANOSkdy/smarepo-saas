...
test('month API returns empty payload when unauthenticated', async () => {
  ...
  assert.deepStrictEqual(await response.json(), { year: 2025, month: 9, days: [] });
  assert.strictEqual(getLogsMock.mock.calls.length, 0);
});

test('month API normalises invalid params to empty result', async () => {
  ...
  assert.strictEqual(response.status, 200);
  assert.deepStrictEqual(await response.json(), { year: 2025, month: 13, days: [] });
});
